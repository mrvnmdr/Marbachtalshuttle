...
import { Calendar, Car, Users, Trash2, Plus, TrendingUp, Download, RefreshCw } from 'lucide-react';
...
const CommuteCostCalculator = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!carsRes.ok || !personsRes.ok || !commutesRes.ok) {
        throw new Error('Fehler beim Laden der Daten');
      }
      
      const [carsData, personsData, commutesData] = await Promise.all([
        carsRes.json(),
        personsRes.json(),
        commutesRes.json()
      ]);
    ...
    if (newCar.ownerId === 'new') {
      ...
      try {
        const res = await fetch('/api/persons', {
          ...
          body: JSON.stringify({ name: newCar.newOwnerName.trim() })
        });
        if (!res.ok) throw new Error('Fehler beim Erstellen der Person');
        const newPersonObj = await res.json();
        setPersons([...persons, newPersonObj]);
        ownerId = newPersonObj.id;
      } catch (err) {
    ...
    try {
      const res = await fetch('/api/cars', {
        ...
        body: JSON.stringify({
          name: newCar.name,
          ownerId: ownerId,
          roundtripCost: parseFloat(newCar.roundtripCost)
        })
      });
      if (!res.ok) throw new Error('Fehler beim Erstellen des Autos');
            ...
            {getMonths().map(month => {
              ...
              return (
                <div key={month} className="mb-6 border rounded-lg p-4">
                      ...
                      {Object.keys(debts).map(passenger => (
                        <div key={passenger}>
                          {Object.keys(debts[passenger]).map(driver => (
                            <div key={`${passenger}-${driver}`} className="flex justify-between p-2 bg-gray-50 rounded text-sm">
                              <span>{passenger} → {driver}</span>
                              <span className="font-medium">{debts[passenger][driver].toFixed(2)} €</span>
                            </div>
                          ))}
                        </div>
                      ))}
                  ...
                  <div>
                    <h4 className="font-medium mb-2 text-green-700">Netto-Abrechnung (nach Verrechnung):</h4>
                    <div className="space-y-2">
                      {Object.keys(netDebts).length > 0 ? (
                        Object.keys(netDebts).map(person1 => (
                          <div key={person1}>
                            {Object.keys(netDebts[person1]).map(person2 => (
                              <div key={`${person1}-${person2}`} className="flex justify-between p-3 bg-green-50 rounded border-l-4 border-green-600">
                                <span className="font-medium">{person1} schuldet {person2}</span>
                                <span className="font-bold text-green-700">{netDebts[person1][person2].toFixed(2)} €</span>
                              </div>
                            ))}
                          </div>
                        ))
                      ) : (
                        <div className="text-gray-500 italic">Keine offenen Schulden</div>
                      )}
                    </div>
                  </div>
                </div>
